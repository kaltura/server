...
class Kaltura_Client_ParseUtils 
{
	...
	public static function checkIfError(\SimpleXMLElement $xml, $throwException = true) 
	{
		if(($xml->error) && (count($xml->children()) == 1))
		{
			...
			$args = self::unmarshalArray($xml->error->args, 'KalturaAPIExceptionArg');
			
			if($throwException)
				throw new Kaltura_Client_Exception($message, $code, $args);
			else 
				return new Kaltura_Client_Exception($message, $code, $args);
		}
	}
}