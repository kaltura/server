...
class JavaClientGenerator extends ClientGeneratorFromXml 
{
	...
	if ($encumCount > 0)
	{
		...
		foreach($enumNode->childNodes as $constNode)
			...
			if ($defaultPropertyName == "") $defaultPropertyName = $propertyName;
	...
	$needsSuperConstructor = false;
        ...
        {
            $this->appendLine("public class $type extends " . $classNode->getAttribute("base") . " {");
	    $needsSuperConstructor = true;
        }
        else
        {
            $this->appendLine("public class $type extends KalturaObjectBase {");
        }
	...
	if ($needsSuperConstructor) $this->appendLine("        super(node);");
	...
	if ($needsArrayList) $imports .= "import java.util.ArrayList;\n";
	if ($needsObjectFactory) $imports .= "import com.kaltura.client.KalturaObjectFactory;\n";
