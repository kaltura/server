...
BATCH_DIR="/opt/kaltura/app/alpha/batch" 
#BATCH_DIR="/opt/kaltura"
LOG_DIR="/var/log" 
# "/opt/kaltura"
RULES_PATH="/opt/kaltura"
...
start() {
	...
	if [ -f $LOCKFILE ]; then
		...
		echo "php $BATCH_DIR/batchRunner.php "php $BATCH_DIR/$FILE_NAME.php" $LOG_DIR/$HOSTNAME-$FILE_NAME.log >> $LOG_DIR/$HOSTNAME-batchRunner.log  2>&1 &"
		php $BATCH_DIR/batchRunner.php "php $BATCH_DIR/$FILE_NAME.php" $LOG_DIR/$HOSTNAME-$FILE_NAME.log >> $LOG_DIR/$HOSTNAME-batchRunner.log  2>&1 &
		if [ "$?" -eq 0 ]; then
			echo_success
			echo
			touch $LOCKFILE	
