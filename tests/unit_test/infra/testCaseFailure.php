...
class testCaseFailure
{
	...
	{
		$testCaseFailure = new testCaseFailure();
	...
	public static function toXml(testCaseFailure $failure, $rootNodeName = 'data')
	{
		...
		foreach ($failure->failures as $unitTestFailure)
		{
			...
			testCaseFailure::setElementValue($xml, $outputReferenceNode, $unitTestFailure->outputReferenceValue, $unitTestFailure->field);

			$actualOutputNode = $xml->createElement("ActualOutput");
			$failureNode->appendChild($actualOutputNode);
			testCaseFailure::setElementValue($xml, $actualOutputNode, $unitTestFailure->actualValue, $unitTestFailure->field);
