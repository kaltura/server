...
FROM  kalturadw.dwh_dim_time t LEFT JOIN (
	SELECT
		date_id, FLOOR(date_id/100) month_id, partner_id, IFNULL(count_bandwidth_kb, 0)/1024 count_bandwidth, IFNULL(aggr_storage_mb, 0) aggr_storage_mb, IFNULL(added_storage_mb, 0) added_storage_mb, IFNULL(deleted_storage_mb, 0) deleted_storage_mb
	FROM
