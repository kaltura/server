...
abstract class kBaseESearch extends kBaseSearch
{
	...
	protected function initQuery(array $statuses, $objectId, kPager $pager = null, ESearchOrderBy $order = null, ESearchAggregations $aggregations=null, $objectIdsNotIn = null)
	{
		...
		$this->initQueryAttributes($partnerId, $objectId, $objectIdsNotIn);
		$this->initBaseFilter($partnerId, $statuses, $objectId, $objectIdsNotIn);
	...
	public static function getSplitIndexNamePerPartner($indexName, $partnerId)
	{
		...
		if(!empty($elasticSplitIndexMap) && isset($elasticSplitIndexMap[$indexName]))
		{
			...
			$index = abs(intval($partnerId / 10) % $splitFactor);
