...
/**
 * Base class that represents a row from the 'virtual_event' table.
 *
 * 
 *
 ...
 * @subpackage model.om
 */
abstract class BaseVirtualEvent extends BaseObject  implements Persistent {
	...
	{
		if(!isset($this->oldColumnsValues[VirtualEventPeer::ID]))
			$this->oldColumnsValues[VirtualEventPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = VirtualEventPeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[VirtualEventPeer::NAME]))
			$this->oldColumnsValues[VirtualEventPeer::NAME] = $this->name;
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = VirtualEventPeer::NAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[VirtualEventPeer::DESCRIPTION]))
			$this->oldColumnsValues[VirtualEventPeer::DESCRIPTION] = $this->description;
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = VirtualEventPeer::DESCRIPTION;
		}
	...
	{
		if(!isset($this->oldColumnsValues[VirtualEventPeer::PARTNER_ID]))
			$this->oldColumnsValues[VirtualEventPeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = VirtualEventPeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[VirtualEventPeer::STATUS]))
			$this->oldColumnsValues[VirtualEventPeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = VirtualEventPeer::STATUS;
		}
	...
	{
		if(!isset($this->oldColumnsValues[VirtualEventPeer::TAGS]))
			$this->oldColumnsValues[VirtualEventPeer::TAGS] = $this->tags;
		...
		if ($this->tags !== $v) {
			$this->tags = $v;
			$this->modifiedColumns[] = VirtualEventPeer::TAGS;
		}
	...
	public function setCustomData($v)
	{
		...
		if ($this->custom_data !== $v) {
			$this->custom_data = $v;
			$this->modifiedColumns[] = VirtualEventPeer::CUSTOM_DATA;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = VirtualEventPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = VirtualEventPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		...
		try {
			...
			return $startcol + 10; // 10 = VirtualEventPeer::NUM_COLUMNS - VirtualEventPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating VirtualEvent object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		VirtualEventPeer::setUseCriteriaFilter(false);
		$criteria = $this->buildPkeyCriteria();
		VirtualEventPeer::addSelectColumns($criteria);
		$stmt = BasePeer::doSelect($criteria, $con);
		VirtualEventPeer::setUseCriteriaFilter(true);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				VirtualEventPeer::doDelete($this, $con);
				$this->postDelete($con);
		...
		try {
			...
			for ($retries = 1; $retries < KalturaPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(VirtualEventPeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = VirtualEventPeer::getAtomicCustomDataFields();
			...
			$this->postSave($con);
			VirtualEventPeer::addInstanceToPool($this);
	...
	protected function doSave(PropelPDO $con, $skipReload = false)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = VirtualEventPeer::doInsert($this, $con);
					if (!$skipReload) {
						$reloadObject = true;
					}
					...
					$affectedObjects = VirtualEventPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = VirtualEventPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = VirtualEventPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = VirtualEventPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(VirtualEventPeer::ID)) $criteria->add(VirtualEventPeer::ID, $this->id);
		if ($this->isColumnModified(VirtualEventPeer::NAME)) $criteria->add(VirtualEventPeer::NAME, $this->name);
		if ($this->isColumnModified(VirtualEventPeer::DESCRIPTION)) $criteria->add(VirtualEventPeer::DESCRIPTION, $this->description);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(VirtualEventPeer::ID, $this->id);
		
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(VirtualEventPeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
				{
					$criteria->add(VirtualEventPeer::CUSTOM_DATA, "MD5(cast(" . VirtualEventPeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					...
					if (kDataCenterMgr::isMultiDc()) // if multi DC configuration don't check costume data on other DC
					{
						...
						$criteria->addOr(VirtualEventPeer::CUSTOM_DATA," '$currentDcId' != getDC()" ,Criteria::CUSTOM);
					}
				}
				else 
					$criteria->add(VirtualEventPeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(VirtualEventPeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != VirtualEventPeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = VirtualEventPeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of VirtualEvent (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     VirtualEvent Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     VirtualEvent Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      VirtualEvent $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(VirtualEvent $copiedFrom)
	{
	 ...
	 *
	 * @return     VirtualEventPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new VirtualEventPeer();
		}
		return self::$peer;
	}
