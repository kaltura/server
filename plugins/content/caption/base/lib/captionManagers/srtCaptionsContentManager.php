...
class srtCaptionsContentManager extends kCaptionsContentManager
{
	...
	{
		$matches = null;
		$regex = '/(?P<index>\d+)\r?\n\s*(?P<startHours>\d{2}):(?P<startMinutes>\d{2}):(?P<startSeconds>\d{2}),(?P<startMilliseconds>\d{1,3})\s*-->\s*(?P<endHours>\d{2}):(?P<endMinutes>\d{2}):(?P<endSeconds>\d{2}),(?P<endMilliseconds>\d{1,3})\s*\r?\n(?P<content>.+)\r?\n\r?(\n|$)/sU';
