	...
	IDistributionEngineReport
{
	...
	{	
		...
		$media->rating = 'TV-'.$this->findMetadataValue($metadataObjects, 'Rating');
		...
		$media->categories = array($distributionProfile->category);
		
		$media->airdate = $entry->createdAt;
		$media->availableDate = date('c', $data->entryDistribution->sunrise);
		if($data->entryDistribution->sunset) {
			$media->expirationDate = date('c', $data->entryDistribution->sunset);
		}
...
//		$media->title = $entry->name;
		$media->title = $this->findMetadataValue($metadataObjects, 'LongTitle');
