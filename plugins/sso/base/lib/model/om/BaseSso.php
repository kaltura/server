...
/**
 * Base class that represents a row from the 'sso' table.
 *
 * 
 *
 * @package plugins.sso
...
abstract class BaseSso extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the application_type field.
	 * @var        string
	 */
	...
	/**
	 * Get the [application_type] column value.
	 * 
	 * @return     string
	 */
	...
	{
		if(!isset($this->oldColumnsValues[SsoPeer::ID]))
			$this->oldColumnsValues[SsoPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SsoPeer::ID;
		}
	...
	/**
	 * Set the value of [application_type] column.
	 * 
	 * @param      string $v new value
	 * @return     Sso The current object (for fluent API support)
	 */
	public function setApplicationType($v)
	{
		if(!isset($this->oldColumnsValues[SsoPeer::APPLICATION_TYPE]))
			$this->oldColumnsValues[SsoPeer::APPLICATION_TYPE] = $this->application_type;

		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->application_type !== $v) {
			$this->application_type = $v;
			$this->modifiedColumns[] = SsoPeer::APPLICATION_TYPE;
		}
	...
	{
		if(!isset($this->oldColumnsValues[SsoPeer::PARTNER_ID]))
			$this->oldColumnsValues[SsoPeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = SsoPeer::PARTNER_ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[SsoPeer::DOMAIN]))
			$this->oldColumnsValues[SsoPeer::DOMAIN] = $this->domain;
		...
		if ($this->domain !== $v) {
			$this->domain = $v;
			$this->modifiedColumns[] = SsoPeer::DOMAIN;
		}
	...
	{
		if(!isset($this->oldColumnsValues[SsoPeer::STATUS]))
			$this->oldColumnsValues[SsoPeer::STATUS] = $this->status;
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = SsoPeer::STATUS;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = SsoPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = SsoPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function setCustomData($v)
	{
		...
		if ($this->custom_data !== $v) {
			$this->custom_data = $v;
			$this->modifiedColumns[] = SsoPeer::CUSTOM_DATA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		...
		try {
			...
			$this->application_type = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
		...
		try {
			...
			for ($retries = 1; $retries < KalturaPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(SsoPeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = SsoPeer::getAtomicCustomDataFields();
			...
			$this->postSave($con);
			SsoPeer::addInstanceToPool($this);
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = SsoPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SsoPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SsoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SsoPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getApplicationType(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setApplicationType($arr[$keys[1]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SsoPeer::ID)) $criteria->add(SsoPeer::ID, $this->id);
		if ($this->isColumnModified(SsoPeer::APPLICATION_TYPE)) $criteria->add(SsoPeer::APPLICATION_TYPE, $this->application_type);
		if ($this->isColumnModified(SsoPeer::PARTNER_ID)) $criteria->add(SsoPeer::PARTNER_ID, $this->partner_id);
		if ($this->isColumnModified(SsoPeer::DOMAIN)) $criteria->add(SsoPeer::DOMAIN, $this->domain);
		if ($this->isColumnModified(SsoPeer::STATUS)) $criteria->add(SsoPeer::STATUS, $this->status);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(SsoPeer::ID, $this->id);
		
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(SsoPeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
				{
					$criteria->add(SsoPeer::CUSTOM_DATA, "MD5(cast(" . SsoPeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					...
					if (kDataCenterMgr::isMultiDc()) // if multi DC configuration don't check costume data on other DC
					{
						...
						$criteria->addOr(SsoPeer::CUSTOM_DATA," '$currentDcId' != getDC()" ,Criteria::CUSTOM);
					}
				}
				else 
					$criteria->add(SsoPeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(SsoPeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != SsoPeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = SsoPeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Sso (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setApplicationType($this->application_type);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Sso Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     Sso Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      Sso $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(Sso $copiedFrom)
	{
