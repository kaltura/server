...
class KOperationEngineExpressionEncoder  extends KSingleOutputOperationEngine
{
	...
	public function __construct($cmd, $destFileName, $outDir)
	{
		...
		KalturaLog::info(": cmd($cmd), destFileName($destFileName), outDir($outDir)");
...
KalturaLog::info(": file_get_contents(configFilePath($this->configFilePath)) => xml($xml)");
		$xml = str_replace(KDLCmdlinePlaceholders::OutDir, $this->outDir, $xml);
KalturaLog::info(": str_replace() => xml($xml)");
