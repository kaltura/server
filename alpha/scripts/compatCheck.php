...
function shouldPassThoughDifferent($resultNew, $resultOld, $key)
{
	$oldValue = $resultOld[$key];
	$newValue = $resultNew[$key];
	
	//drm_drmlicenseaccess.getaccess
	if ( ($key === 'duration' || $key === 'absolute_duration') &&
		isset($resultOld['objectType']) && ($resultOld['objectType'] === 'KalturaDrmLicenseAccessDetails') &&
		isset($resultNew['objectType']) && ($resultNew['objectType'] === 'KalturaDrmLicenseAccessDetails') &&
		($oldValue - $newValue == 1) )
	{
		print "Allow for drm_drmlicenseaccess.getaccess since differ in 1 second only: field ($key), old ($oldValue) new ($newValue)";
		return true;
	}
	...
	foreach ($resultOld as $key => $oldValue)
	{
		...
		else if (is_string($oldValue) && is_string($newValue))
		{
			if (!compareValues($newValue, $oldValue))
			{
				...
				else if(shouldPassThoughDifferent($resultNew, $resultOld, $key))
				{
					continue;
				}
				else
...
function testAction($ipAddress, $fullActionName, $parsedParams, $uri, $postParams = array(), $compareMode = CM_XML, $kalcliCmd = '')
{
	...
	for ($tries = 1; $tries <= 3; $tries++)
	{
		...
		{
			print 'Empty/False result was received from new server';
			$errors = array('Empty/False result was received from new server');
			usleep(1000000 * 45); //45 seconds
			continue;
		}
