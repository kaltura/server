...
class deleteFilesWorker
{
	...
	const EFS_FILE_PATH_PARAM = 'efs_file_path';
	const EFS_DC = 1;
	const AWS_DC = 2;
	const MIN_FILE_SIZE_PARAM = 'min_file_size';
	...
	protected function getCandidatesForDelete($minFileSyncId)
	{
		...
		$efsFilePath = kConf::get(self::EFS_FILE_PATH_PARAM,self::MAP_NAME, '%r74v1%');
		$minFileSize = kConf::get(self::MIN_FILE_SIZE_PARAM,self::MAP_NAME, 100*1024*1024);
		...
		$criterion  = $criteria->getNewCriterion(FileSyncPeer::ID, $minFileSyncId, Criteria::GREATER_EQUAL);
		...
		$criteria->add(FileSyncPeer::FILE_PATH, $efsFilePath, Criteria::LIKE);
		$criteria->add(FileSyncPeer::DC, self::EFS_DC, Criteria::EQUAL);
		$criteria->add(FileSyncPeer::FILE_SIZE, $minFileSize,Criteria::GREATER_THAN);
	...
	protected function getMinFileSyncIdQuery($minUpdatedAt)
	{
		...
		$criterion2 = $criteria->getNewCriterion(FileSyncPeer::UPDATED_AT,$minUpdatedAt + self::MIN_FILE_SYNC_ID_TIME_WINDOW,Criteria::LESS_THAN);
		$criteria->addAnd($criterion);
		$criteria->addAnd($criterion2);
		$criteria->add(FileSyncPeer::OBJECT_TYPE, FileSyncObjectType::ASSET);
		$criteria->add(FileSyncPeer::OBJECT_SUB_TYPE, flavorAsset::FILE_SYNC_ASSET_SUB_TYPE_ASSET);
		$criteria->add(FileSyncPeer::STATUS, self::FILE_SYNC_READY_STATUS, Criteria::EQUAL);
		$criteria->add(FileSyncPeer::FILE_PATH, $efsFilePath, Criteria::LIKE);
	...
	protected function getMinFileSyncId()
	{
		...
		$minTime = time() - self::TWO_DAYS_IN_SECONDS;
		$fileSync = null;
		$maxUpdatedAt = kConf::get(self::MAX_UPDATED_AT_PARAM,self::MAP_NAME, 12*3600);
...
$deleteFilesWorker = new deleteFilesWorker($dryRun);
