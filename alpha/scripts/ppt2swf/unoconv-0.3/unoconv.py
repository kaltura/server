...
class Convertor:
    def __init__(self):
        ...
        try:
            ...
            try:
#                print "*** RESTARTING soffice daemon"
#                os.spawnvp(os.P_NOWAIT, "/opt/kaltura/ppt2swf/start_openoffice_daemon.sh", ["/opt/kaltura/ppt2swf/start_openoffice_daemon.sh"])
#                time.sleep(5)
                unocontext = resolver.resolve("uno:%s" % op.connection)
            except Exception, e:
...
def main():
    ...
    try:
        ...
        for inputfn in op.filenames:
            myex = convertor.convert(inputfn)
