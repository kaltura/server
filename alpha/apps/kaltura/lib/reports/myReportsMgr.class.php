...
class myReportsMgr
{
	...
	$page_size , $page_index  , $order_by , $obj_ids_clause = null )
	{
		...
		$search_text_match_clause = self::setSearchFieldsAndText ( $input_filter, $partner_id );
	 ...
	 */
	private static function setSearchFieldsAndText ( reportsInputFilter $input_filter, $partner_id)
	{
...
/*		$criteria->add(entryPeer::STATUS,entryStatus::READY);
		$criteria->add(entryPeer::TYPE,array(entryType::MEDIA_CLIP, entryType::MIX), Criteria::IN);
		$criteria->add(entryPeer::MODERATION_STATUS, array(
		entry::ENTRY_MODERATION_STATUS_REJECTED,
		entry::ENTRY_MODERATION_STATUS_PENDING_MODERATION), Criteria::IN);
*/

		if ( $input_filter->search_in_tags ) //== "true" )
				$criteria->add($criteria->getNewCriterion(entryPeer::SEARCH_TEXT, $search_text, Criteria::LIKE));
			//$search_fields = "en.entry_name,en.description,en.tags,en.admin_tags" ;// use index for all searchable fields
		else
			$criteria->add(entryPeer::ADMIN_TAGS, $search_text, Criteria::LIKE);
			//$search_fields = "en.admin_tags" ;// use index for admin_tags only

		//$result = entryPeer::doSelect($criteria);
		$criteria->applyFilters();
		$entryIds = $criteria->getFetchedIds();

		$result = "(en.entry_id) IN ('".implode("','", $entryIds)."')";
