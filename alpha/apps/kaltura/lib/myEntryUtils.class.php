...
*/
	...
	public static function deleteEntry ( entry $entry , $partner_id = null , $onlyIfAllJobsDone = false)
	{
		...
		if($entry->getType() === entryType::LIVE_STREAM)
		{
			...
			if($entry->getRecordedEntryId())
			{
				...
				if($recordedEntry)
				{
					...
					if($recordedEntry->getCreatedAt(null) > time() - dateUtils::DAY * 3)
					{
		...
		if($entry->getSourceType() == EntrySourceType::KALTURA_RECORDED_LIVE)
		{
			...
			//If entry is pending for recording to finish for more than 3 days than it will probably never happen
			if($entry->getCreatedAt(null) > time() - dateUtils::DAY * 3 && myEntryUtils::shouldServeVodFromLive($entry, false))
			{
