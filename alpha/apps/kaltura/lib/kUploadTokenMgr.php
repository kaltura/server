...
class kUploadTokenMgr
{
	...
	protected function handleResume($fileData, $resumeAt)
	{
		...
		if (!kFile::checkFileExists($uploadFilePath) && !kFile::isSharedPath($uploadFilePath))
		...
		if ($resumeAt != -1) // this may not be a sequential chunk added at the end of the file
		{
			...
			KalturaLog::debug("uploadStats {$this->_uploadToken->getId()}: $resumeAt $chunkSize $currentFileSize " . $verifyFinalChunk . " " . kCurrentContext::$client_lang);
	...
	static protected function appendChunks($targetFileResource, $chunks)
	{
		...
		for (;;)
		{
			...
			if(!kFile::checkFileExists($nextChunk))
			{
