...
class EntrySphinxCriterion extends Criterion
{
	/**
	 * @var bool
	 */
	...
	public function apply(array &$whereClause, array &$matchClause)
	{
		...
		if(count($clauses))
		{
			...
			{
				if(!($clause instanceof EntrySphinxCriterion))
				{
		...
		if(!isset(entryFilter::$sphinxFields[$field]))
		{
		...
		$sphinxField = entryFilter::getSphinxFieldName($field);
