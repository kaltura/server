...
class entry extends Baseentry implements ISyncableFile
{
	...
	const ENTRY_SPHINX_TYPE_AUTOMATIC = 101;
	...
	const ENTRY_SPHINX_STATUS_ERROR_IMPORTING = 102;
	const ENTRY_SPHINX_STATUS_ERROR_CONVERTING = 101;
...
//		if ( $v === null ) return ;
	 ...
	 * @return     int
	 */
	public function getSphinxId()
	{
		return self::translateSphinxId($this->getId());
	}

	public static function translateSphinxId($id)
	{
		return crc32($id);
	}
...
// ----------- Extra object connections ----------------
	...
	public function saveToSphinxRequired()
	{
		$cols = $this->getSphinxFields();
		foreach($cols as $col)
	...
	public function getSphinxSaveSql($isInsert = false, $force = false)
	{
		$id = $this->getIntId();
		if(!$id)
		{
			KalturaLog::err("Entry [" . $this->getId() . "] could not be saved to sphinx, int_id is empty");
			return false;
		}
		
		if(!$force && !$isInsert && !$this->saveToSphinxRequired())
			return false;
			
		$categoriesIds = explode(',', $this->getCategoriesIds());
		$categories = implode(',', $categoriesIds);
		
		$flavorParamsIds = explode(',', $this->getFlavorParamsIds());
...
//		if($isInsert)
			$dataStrings['entry_id'] = $this->getId();
			$dataStrings['str_entry_id'] = $this->getId();
		
//		if($isInsert || $this->isColumnModified(entryPeer::NAME))
			$dataStrings['name'] = $this->getName();
			$dataInts['sort_name'] = kUTF8::str2int64($this->getName());
			
//		if($isInsert || $this->isColumnModified(entryPeer::TAGS))
			$dataStrings['tags'] = $this->getTags();
//		if($isInsert || $this->isColumnModified(entryPeer::CATEGORIES_IDS))
			$dataStrings['categories'] = $categories;
//		if($isInsert || $this->isColumnModified(entryPeer::FLAVOR_PARAMS_IDS))
			$dataStrings['flavor_params'] = $flavorParams;
//		if($isInsert || $this->isColumnModified(entryPeer::SOURCE_LINK))
			$dataStrings['source_link'] = $this->getSourceLink();
//		if($isInsert || $this->isColumnModified(entryPeer::KSHOW_ID))
			$dataStrings['kshow_id'] = $this->getKshowId();
//		if($isInsert || $this->isColumnModified(entryPeer::GROUP_ID))
			$dataStrings['group_id'] = $this->getGroupId();
//		if($isInsert || $this->isColumnModified(entryPeer::DESCRIPTION))
...
//		{
			$pluginsData = $this->getPluginData();
			...
			try
			{
				...
				$sphinxPluginData = kMetadataManager::getSearchValuesByObject(Metadata::TYPE_ENTRY, $this->getId());
				if($sphinxPluginData)
				{
...
//		if($isInsert)
			$dataInts['int_entry_id'] = $this->getSphinxId();
//		if($isInsert || $this->isColumnModified(entryPeer::KUSER_ID))
			$dataInts['kuser_id'] = $this->getKuserId();
//		if($isInsert || $this->isColumnModified(entryPeer::STATUS))
			$dataInts['entry_status'] = $this->getStatus();
//		if($isInsert || $this->isColumnModified(entryPeer::TYPE))
			$dataInts['type'] = $this->getType();
//		if($isInsert || $this->isColumnModified(entryPeer::MEDIA_TYPE))
			$dataInts['media_type'] = $this->getMediaType();
//		if($isInsert || $this->isColumnModified(entryPeer::VIEWS))
			$dataInts['views'] = $this->getViews();
//		if($isInsert || $this->isColumnModified(entryPeer::PARTNER_ID))
			$dataInts['partner_id'] = $this->getPartnerId();
//		if($isInsert || $this->isColumnModified(entryPeer::MODERATION_STATUS))
			$dataInts['moderation_status'] = $this->getModerationStatus();
//		if($isInsert || $this->isColumnModified(entryPeer::DISPLAY_IN_SEARCH))
			$dataInts['display_in_search'] = $this->getDisplayInSearch();
//		if($isInsert || $this->isColumnModified(entryPeer::LENGTH_IN_MSECS))
			$dataInts['duration'] = $this->getDurationInt();
//		if($isInsert || $this->isColumnModified(entryPeer::ACCESS_CONTROL_ID))
			$dataInts['access_control_id'] = $this->getAccessControlId();
//		if($isInsert || $this->isColumnModified(entryPeer::MODERATION_COUNT))
			$dataInts['moderation_count'] = $this->getModerationCount();
//		if($isInsert || $this->isColumnModified(entryPeer::RANK))
			$dataInts['rank'] = $this->getRank();
//		if($isInsert || $this->isColumnModified(entryPeer::PLAYS))
			$dataInts['plays'] = $this->getPlays();
		
//		if($isInsert || $this->isColumnModified(entryPeer::CREATED_AT))
			$dataTimes['created_at'] = $this->getCreatedAt(null);
//		if($isInsert || $this->isColumnModified(entryPeer::UPDATED_AT))
			$dataTimes['updated_at'] = $this->getUpdatedAt(null);
//		if($isInsert || $this->isColumnModified(entryPeer::MODIFIED_AT))
			$dataTimes['modified_at'] = $this->getModifiedAt(null);
//		if($isInsert || $this->isColumnModified(entryPeer::MEDIA_DATE))
			$dataTimes['media_date'] = $this->getMediaDate(null);
//		if($isInsert || $this->isColumnModified(entryPeer::START_DATE))
			$dataTimes['start_date'] = $this->getStartDate(null);
//		if($isInsert || $this->isColumnModified(entryPeer::END_DATE))
			$dataTimes['end_date'] = $this->getEndDate(null);
//		if($isInsert || $this->isColumnModified(entryPeer::AVAILABLE_FROM))
			$dataTimes['available_from'] = $this->getAvailableFrom(null);
	...
	public function updateSphinx($field, $value, $type = PDO::PARAM_STR)
	{
		$index = entryPeer::SPHINX_INDEX_NAME;
		...
		switch($type)
		{
			...
			case PDO::PARAM_INT:
				if($value < 0)
					throw new Exception("Entry [" . $this->getId() . "] field [$field] can't be negative [$value]");
				break;
	...
	public function execSphinx($sql)
	{
		...
		$sphinxLog->setEntryId($this->getId());
		$sphinxLog->setPartnerId($this->getPartnerId());
	...
	// TODO remove $force after replace bug solved
	public function saveToSphinx($isInsert = false, $force = false)
	{
		$sql = $this->getSphinxSaveSql($isInsert, $force);
		if(!$sql)
			return;
