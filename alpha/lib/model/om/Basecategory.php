...
abstract class Basecategory extends BaseObject  implements Persistent {
		...
		try {
			...
			for ($retries = 1; $retries < KalturaPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(categoryPeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = categoryPeer::getAtomicCustomDataFields();
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(categoryPeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
					$criteria->add(categoryPeer::CUSTOM_DATA, "MD5(cast(" . categoryPeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					//casting to latin char set to avoid mysql and php md5 difference
				else 
					$criteria->add(categoryPeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(categoryPeer::UPDATED_AT))
			{
