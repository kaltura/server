...
abstract class BaseConversionParams extends BaseObject  implements Persistent {
		...
		try {
			...
			for ($retries = 1; $retries < KalturaPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(ConversionParamsPeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
				...
				$validUpdate = true;
				$atomicCustomDataFields = ConversionParamsPeer::getAtomicCustomDataFields();
	...
	public function buildPkeyCriteria()
	{
		...
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(ConversionParamsPeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
					$criteria->add(ConversionParamsPeer::CUSTOM_DATA, "MD5(cast(" . ConversionParamsPeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					//casting to latin char set to avoid mysql and php md5 difference
				else 
					$criteria->add(ConversionParamsPeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(ConversionParamsPeer::UPDATED_AT))
			{
