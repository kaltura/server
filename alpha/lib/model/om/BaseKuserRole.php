...
/**
 * Base class that represents a row from the 'kuser_role' table.
 *
 * 
 *
...
abstract class BaseKuserRole extends BaseObject  implements Persistent {
	 ...
	 * @return     KuserRole The current object (for fluent API support)
	 */
	public function setId($v)
	{
		if(!isset($this->oldColumnsValues[KuserRolePeer::ID]))
			$this->oldColumnsValues[KuserRolePeer::ID] = $this->id;
	...
	{
		if(!isset($this->oldColumnsValues[KuserRolePeer::KUSER_ID]))
			$this->oldColumnsValues[KuserRolePeer::KUSER_ID] = $this->kuser_id;
	...
	{
		if(!isset($this->oldColumnsValues[KuserRolePeer::USER_ROLE_ID]))
			$this->oldColumnsValues[KuserRolePeer::USER_ROLE_ID] = $this->user_role_id;
		...
		if ($this->user_role_id !== $v) {
			$this->user_role_id = $v;
			$this->modifiedColumns[] = KuserRolePeer::USER_ROLE_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = KuserRolePeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = KuserRolePeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = KuserRolePeer::NUM_COLUMNS - KuserRolePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = KuserRolePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				KuserRolePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		KuserRolePeer::setUseCriteriaFilter(false);
		$this->reload();
		KuserRolePeer::setUseCriteriaFilter(true);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(KuserRolePeer::USER_ROLE_ID)) $criteria->add(KuserRolePeer::USER_ROLE_ID, $this->user_role_id);
		if ($this->isColumnModified(KuserRolePeer::CREATED_AT)) $criteria->add(KuserRolePeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(KuserRolePeer::UPDATED_AT)) $criteria->add(KuserRolePeer::UPDATED_AT, $this->updated_at);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of KuserRole (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     KuserRole Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     KuserRole Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      KuserRole $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(KuserRole $copiedFrom)
	{
	...
	public function getkuser(PropelPDO $con = null)
	{
		if ($this->akuser === null && ($this->kuser_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->akuser->addKuserRoles($this);
			 */
		}
		return $this->akuser;
	}
	...
	public function getUserRole(PropelPDO $con = null)
	{
		if ($this->aUserRole === null && ($this->user_role_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aUserRole->addKuserRoles($this);
			 */
		}
