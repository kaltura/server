...
class DeliveryProfileDynamicAttributes {
	...
	private function isPlayerConfigValid($playerConfig)
	{
		...
		{
			KalturaLog::debug("playerConfig provided is not a json object, data will not be forwarded to playServer [$playerConfig]");
			return false;
		}
		
		if(isset($playConfigJson->sessionId) && is_int($playConfigJson->sessionId))
		{
			KalturaLog::debug("Integer sessionId value provided in player config, data will not be forwardrd to playServer [$playerConfig]");
			return false;
		}
