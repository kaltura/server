...
class UserService extends KalturaBaseUserService 
{
	 ...
	 * @param string $loginId The user's email address that identifies the user for login
	 * @param string $password The user's password
	 * @param string $userActivationKey User activation key generated by the system for assigning existing user to a new account
	 ...
	 *
	 */	
	public function enableLoginAction($userId, $loginId, $password = null, $userActivationKey = null)
	{		
		try
		{
			...
			$user->enableLogin($loginId, $password, true, null, $userActivationKey);
			$user->save();
		}
		catch (Exception $e)
		{
