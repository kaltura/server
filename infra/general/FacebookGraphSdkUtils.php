...
require_once KALTURA_ROOT_PATH.'/vendor/facebook-sdk-php-v5/autoload.php';
...
class FacebookGraphSdkUtils
{
	 ...
	 */
	public static function getLongLivedUserAccessToken($appId, $appSecret, $permissions = array())
	{
		$fb = new Facebook\Facebook(array ('app_id' => $appId, 'app_secret' => $appSecret));
	 ...
	 * @throws Facebook\Exceptions\FacebookResponseException, Facebook\Exceptions\FacebookSDKException, Exception
	 */
	public static function getUserAccessToken($appId, $appSecret, $permissions = array())
	{
		$fb = new Facebook\Facebook(array ('app_id' => $appId, 'app_secret' => $appSecret));
	  	$userAccessToken = self::doGetUserAccessToken($fb, $appId, $permissions);
		if(!isset($userAccessToken))
	 ...
	 * @throws Facebook\Exceptions\FacebookResponseException, Facebook\Exceptions\FacebookSDKException, Exception
	 */
	public static function getPageAccessToken($appId, $appSecret, $userAccessToken, $pageId)
	{
		$fb = new Facebook\Facebook(array ('app_id' => $appId, 'app_secret' => $appSecret));
	 ...
	 * @param unknown_type $permissions
	 * @param unknown_type $reRequestPermissions
	 */
	public static function getLoginUrl($appId, $appSecret, $redirectUrl, $permissions, $reRequestPermissions = false)
	{
		$fb = new Facebook\Facebook(array ('app_id' => $appId, 'app_secret' => $appSecret));
	...
	{
		KalturaLog::debug('Getting user access token');
		...
		KalturaLog::debug('User access token: ' . $accessToken->getValue(). ' expiration: '.$accessToken->getExpiresAt());
